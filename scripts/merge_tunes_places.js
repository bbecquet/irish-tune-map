...
function run() {
  ...
  const tunesForMatching = tunes.map(tune =>
    R.assoc('namesMatchString', getTuneMatchingString(tune), tune)
  )

  let counter = 0
  const addTunes = place => {
    console.log(++counter, place.properties.name)
    const matchingTunes = findMatchingTunes(place, tunesForMatching).map(
      R.dissoc('namesMatchString')
    )
  ...
  fs.writeFileSync(files.out, JSON.stringify(resultGeoJson, null, 2))
